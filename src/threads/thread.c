...
bool priority_comp(const struct list_elem* left, const struct list_elem* right, void* aux){
	struct thread *l;
	struct thread *r;
	l = list_entry(left, struct thread, elem);
	r = list_entry(right, struct thread, elem);
...
void update_load_avg_recent_cpu(void){
	...
	load_avg = float_int_div(float_int_add(int_float_mul(59, load_avg), ready_size), 60);

	for(e = list_begin(&all_list);e != list_end(&all_list); e = list_next(e)){
		t = list_entry(e, struct thread, allelem);
		if(t != idle_thread){
			t->recent_cpu = float_int_add(float_float_mul(float_float_div(int_float_mul(2, load_avg), float_int_add(int_float_mul(2,load_avg), 1)), t->recent_cpu), t->nice);
		}
	}
...
void update_priority(void){
	...
	for(e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)){
		t = list_entry(e, struct thread, allelem);
		t -> priority = float_float_sub(float_float_sub(float_int_add(0, PRI_MAX), float_int_div(t->recent_cpu, 4)), int_float_mul(2, float_int_add(0, t->nice)))/FRAC;
...
{
  /* Not yet implemented. */
  return int_float_mul(100,load_avg)/FRAC;
}
...
{
  /* Not yet implemented. */
  return int_float_mul(100,thread_current()->recent_cpu)/FRAC;
}

